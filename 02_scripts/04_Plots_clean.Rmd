...
Weighted <- ggplot(results3, aes(x = m, y = d, color = d)) + 
  ...
  geom_hline(data = results4, aes(yintercept = wtd_hi), linetype = 2, alpha = 0.5) +
    geom_text(data = results4, 
            aes(x = 20, y = wtd_lo, label = sprintf("%.1f", wtd_lo)), 
            vjust = 0.8, color = "black") +  
  geom_text(data = results4, 
            aes(x = 20, y = wtd_hi, label = sprintf("%.1f", wtd_hi)), 
            vjust = -1.2, color = "black") + 
  
  geom_text(data = results4, 
            aes(x = 75, y = wgt_mean, label = sprintf("%.1f", wgt_mean)), 
            vjust = -0.4, color = "black") +
  
  coord_cartesian(xlim = c(20, 80)) +
  labs(title = "Means and Limits of Agreement", 
       x = "PROMIS T-Score", 
       y = "Difference (d)",   
       subtitle = "Weighted by groupsize") + 
  scale_color_viridis_c(option = "D") + 
  theme_minimal() +
  theme(
...
geom_text(data = results4, 
          ...
          vjust = 0.2, hjust = 3.1, color = "black", size = 4.5) +  
  geom_text(data = results4, 
            aes(x = 50, y = wtd_hi, label = sprintf("%.1f", wtd_hi)), 
            vjust = 0.8, hjust = 3.1, color = "black", size = 4.5) + 
  geom_text(data = results4, 
