...
result_df_groupsWithinStudy <- result_df_groupsWithinStudy %>%
  distinct(Study, Value) %>%
  rename(groupsWithinStudy = Value)

# Tippfehler aus Datensatz löschen

##result_df_studytype <- result_df_studytype %>%
#  distinct(Study, Value)
#result_df_studytype <- result_df_studytype[-c(40, 78), ]  %>%
#  rename(studytype = Value)

result_df_measurementTimePoints <- result_df_measurementTimePoints %>%
  distinct(Study, Value) %>%
  rename(measurementTimePoints = Value)



table1_df <- result_df_age %>%
  left_join(result_df_country, by = "Study") %>%
  left_join(result_df_gender, by = "Study") %>%
  left_join(result_df_groupsWithinStudy, by = "Study") %>%
#  left_join(result_df_studytype, by = "Study") %>%
  left_join(result_df_measurementTimePoints, by = "Study")

library(table1)

label(table1_df$age) <- "Age (years)"
label(table1_df$measurementTimePoints) <- "Measurement Timepoints within studies"
label(table1_df$gender) <- "Females in Percent"

table1(~ age + gender + measurementTimePoints, data = table1_df)
